...
from app.schemas.jobs.post_jobs_response import (
    Areas,
    CommutingTime,
...
@router.post(
    "",
    response_model=PostJobsResponse,
    summary="スカウト求人作成",
)
async def post_jobs(
    job: Job,
    ...
    framework_list = []
    if job.frameworks:
        for pl_id in job.frameworks:
            jobs_framework = Frameworks(framework_id=pl_id, framework_name="string")
